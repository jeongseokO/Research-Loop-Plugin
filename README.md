...
현재 패키지: **0.9.0**. 할 일을 전체 목표 → 월간 → 주간 → 실행 항목으로 연결하고, 자기 주인의 담당 작업과 상위 목표를 함께 확인합니다. 중간 단계는 생략할 수 있습니다. 담당자·완료 기준·미팅 출처는 각 항목에 유지되며, 하위 작업 완료가 상위 목표를 자동으로 완료하지 않습니다. 사람이 수락하고 최종 완료를 확인합니다. 작은 요약과 필요한 목표 경로만 조회하며, 반복 polling이나 종료된 AI의 자동 실행은 하지 않습니다. 기존 작업 지침·검토·경량 조회·자원 제한은 유지합니다.

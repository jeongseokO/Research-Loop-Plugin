...
현재 패키지: **0.14.1**. 연구 트리는 **요약 → 관련 가지 → 근거 페이지 → 바뀐 부분만 반영** 순서로 정리합니다. `patch_research_tree`가 다른 걸음과 생략한 필드를 보존합니다. 전체 교체는 전체 curation을 읽은 경우에만 허용하며, Editor의 첫 생성·본인 작성 트리도 승인 요청입니다. 핵심 결과·판단은 짧게, 상세 근거는 원본 페이지 링크로 연결합니다. 일반 페이지·할 일·일정·그림 지침은 필요한 작업에서만 읽고 기존 시각화 스타일과 개인정보 보호 원칙을 유지합니다.

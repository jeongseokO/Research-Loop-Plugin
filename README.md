...
현재 패키지: **0.10.0**. 배정 업무 하나의 목표·결과·진행 상황을 좁게 조회합니다. AI의 결과 확인 요청은 실제 저장된 업무·결과 버전과 후속 검토를 함께 기록하며, 완료는 사람이 확인합니다. 공통 지침과 업무·편집·첨부·AI 요청 지침을 분리하여 필요한 안내만 읽습니다. 반복 polling·자동 실험 실행·전체 프로젝트 재조회는 하지 않습니다. 전체 목표 → 월간 → 주간 → 실행 항목의 계층과 기존 권한·승인 규칙은 유지합니다.

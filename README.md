...
현재 패키지: **0.14.2**. 하이라이트 AI 요청은 특정 AI 선택 없이 내 AI 공용 대기열에 들어갑니다. 첫 claim만 실행하고 검토 대기·완료된 요청은 중복 처리하지 않습니다.  연구 트리는 **요약 → 관련 가지 → 근거 페이지 → 바뀐 부분만 반영** 순서로 정리합니다. `patch_research_tree`가 다른 걸음과 생략한 필드를 보존합니다. 전체 교체는 전체 curation을 읽은 경우에만 허용하며, Editor의 첫 생성·본인 작성 트리도 승인 요청입니다. 핵심 결과·판단은 짧게, 상세 근거는 원본 페이지 링크로 연결합니다. 일반 페이지·할 일·일정·그림 지침은 필요한 작업에서만 읽고 기존 시각화 스타일과 개인정보 보호 원칙을 유지합니다.
...
- 로컬 Python·Matplotlib으로 PNG와 수정용 JSON을 만듭니다. SVG/PDF는 선택 출력입니다. 별도 이미지 생성 API는 사용하지 않습니다. 환경이 없으면 [전용 환경 설정](plugins/research-loop/skills/method-figure/references/setup.md)이 필요합니다.

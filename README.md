...
현재 패키지: **0.13.1**. 할 일의 본문은 미팅에서 설명할 수 있는 분석으로 작성하고, 진행 로그는 분리합니다. 전용 분석 양식은 해당 작업에서만 읽습니다. 기록 목적에 맞는 연구 탭을 사용하고, 데이터셋 예시는 한 곳에 보관해 링크로 참조하도록 안내합니다. 정량 비교는 기존 공통 스타일의 핵심 그래프로 설명하되 전체 수치와 근거를 보존합니다. 필요한 항목만 읽는 경량 지침과 기존 승인·버전 확인·업무 진행 보고·일정 개인정보 보호 규칙은 유지합니다.

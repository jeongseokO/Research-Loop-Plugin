...
3. The renderer produces PNG, SVG, PDF and the source JSON, plus a small manifest with the PNG's actual size and SHA-256. Keep these files for later revisions. Reuse the source for local changes rather than recreating the entire figure. Do not load source code or image base64 into context when a file path suffices.

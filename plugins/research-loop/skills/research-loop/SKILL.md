...
4. For authorized project work, check relevant queued AI requests once via `list_ai_tasks`; null assignment means the human owner's AI pool. Read [AI work](references/ai-work.md) only when handling one; claim before execution and never redo claimed/review/completed work. Do not poll.
5. Open only relevant blocks at the returned page revision. Summaries and partial lists are not complete evidence. History tools are for required deltas/reconstruction, not startup replay.

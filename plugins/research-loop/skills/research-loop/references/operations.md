...
- After commits, inspect affected records and `readingGuides` via `get_research_brief`. Follow `get_research_workflow_guide` to scale review to scientific impact. Wording/layout edits do not authorize a broad rewrite or skipping stale-version checks. Use `get_research_review` or `get_reading_guide` for exact versions; acknowledge only evidence actually read. Unchecked dependencies and pending proposals remain pending.

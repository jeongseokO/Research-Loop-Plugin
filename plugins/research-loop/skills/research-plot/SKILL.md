...
- Absolute bars/areas start at zero; deltas include zero. For close high scores, use paired dots, a meaningful line plot or delta panel. Points/lines may use clearly labeled tighter bounds containing all relevant values/intervals with padding; disclose nonzero bounds in the caption. Never crop bars, omit inconvenient observations, reverse axes silently or imply significance from zoom. Comparable panels share units and ranges. Use stable entity colors; signed blue/orange encodes direction, not automatically good/bad.
